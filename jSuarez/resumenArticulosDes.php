...
<body>
    ...
    ?>
    <main>
    <h2>Resumen de Artículos Deshabilitados Vendidos</h2>
    <div class="div-form-estado">
    <form action="resumenArticulosDes.php" method="POST">
        <div>
        ...
        <div>
        <button type="submit" name="consultar_articulos" class="boton">Consultar Artículos</button>
    ...
    <?php
if (isset($_POST['consultar_articulos']) && isset($username) && $rol == "root") {
    ...
    $ventasPorArticulo = $sqlVenta->getVentasPorArticulo($fecha_inicio, $fecha_fin, 0);

    if (!empty($ventasPorArticulo)) {
        echo '<div id="printableArea">';
        echo '<h3>Resumen de Ventas por Artículos Deshabilitados</h3>';
        echo '<p class="parrafos-estadocuenta">Desde: ' . $fecha_inicio . ' --- Hasta: ' . $fecha_fin . '</p>';
        echo '<p class="parrafos-estadocuenta">Registros encontrados: ' . count(mysqli_fetch_all($ventasPorArticulo)) . '</p>';
        echo '<div class="table-container">';
        echo '<table class="tablaExistencias" id="miTabla">';
        echo '<tr><th>ID Tienda</th><th>Tienda</th><th>Cantidad</th><th>Total</th></tr>';
        $total = 0;
        $cant_total = 0;
        foreach ($ventasPorArticulo as $articulo) {
            echo '<tr>';
            echo '<td>' . $articulo['idArticulo'] . '</td>';
            echo '<td>' . $articulo['descripcion'] . '</td>';
            echo '<td>' . $articulo['cantidad_ventas'] . '</td>';
            echo '<td>$' . $articulo['total_ventas'] . '</td>';
            echo '</tr>';
            $total += $articulo['total_ventas'];
            $cant_total += $articulo['cantidad_ventas'];
        }
        echo '<tr>';
